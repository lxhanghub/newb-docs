---
home: true
heroImage: /img/newb_logo.png
heroText: hello newb
tagline: 🚀内置依赖注入、配置管理、日志模块、领域驱动设计
actionText: 开始使用 →
...
features: # 可选的
  - title: 依赖注入
    details: 基于 Uber Fx 理念，通过 fx.Option 管理服务依赖
  - title: 配置管理
    details: 基于 Viper 封装支持 YAML、json、ENV环境变量、命令行多源加载
  - title: 日志模块
    details: 基于 Zap，极致性能、JSON结构化日志（Prod模式）多目标输出：控制台 + 文件
